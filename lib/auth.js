...
async function validateToken(token) {
    ...
    try {
        ...
        // SECURITY: prefer HMAC-style shared secret. Falls back to the legacy
        // X-Internal-Request header, which the PHP side now restricts to
        // loopback / private-network callers only.
        const headers = {
            'X-Internal-Request': 'true',
            'Content-Type': 'application/json'
        };
        if (config.api.internalSecret) {
            headers['X-Internal-Secret'] = config.api.internalSecret;
        }
